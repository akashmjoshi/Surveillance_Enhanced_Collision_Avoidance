...
struct background bg_c_frame[Pixels*2];
...
#pragma DATA_SECTION(bg_y_frame, ".user")
...
void bg_y_calculation(){
	Int32 i, j, k =0;
	Uint8 y1,y0;
	Uint32 d1,d0;
	float p0 = 0.01, p1 = 0.99;
	int index = 0;
	...
	for(i=0; i<vHeight; ++i){
		for (j=0; j<vWidth/2; j+=1) {
			...
			bg_y_frame[k].mean = (p0 * y0) + (p1 * bg_y_frame[k].mean);
			bg_y_frame[k+1].mean = (p0 * y0) + (p1 * bg_y_frame[k+1].mean);
			
			d0 = (y0 - bg_y_frame[k].mean) * (y0 - bg_y_frame[k].mean) ;
			d1 = (y1 - bg_y_frame[k+1].mean) * (y1 - bg_y_frame[k+1].mean) ;
			
			bg_y_frame[k].var = (p0 * d0) + (p1 * bg_y_frame[k].var);
			bg_y_frame[k+1].var = (p0 * d1) + (p1 * bg_y_frame[k+1].var);
			
			k = k + 2;
		}
	}
}
void bg_color_calculation(){
	Int32 i, j, k =0;
	Uint8 cr,cb;
	Uint32 d1,d0;
	float p0 = 0.01, p1 = 0.99;
	...
	for(i=0; i<vHeight; ++i){
		for (j=0; j<vWidth/2; j+=1) {
			...
			bg_c_frame[k].mean = (p0 * cb) + (p1 * bg_c_frame[k].mean);
			bg_c_frame[k+1].mean = (p0 * cr) + (p1 * bg_c_frame[k+1].mean);
			
			d0 = (cb - bg_c_frame[k].mean) * (cb - bg_c_frame[k].mean) ;
			d1 = (cr - bg_c_frame[k+1].mean) * (cr - bg_c_frame[k+1].mean) ;
			
			bg_c_frame[k].var = (d0 * p0) + (bg_c_frame[k].var * p1);
			bg_c_frame[k+1].var = (d1 * p0) + (bg_c_frame[k+1].var * p1);
...
void bg_subtraction(){
	Int32 i, j;
	Uint8 cr_buf_in, cb_buf_in;
	Uint8 y1_buf_in, y0_buf_in;
	Int32 diff_y1,diff_y0;
	Int32 diff_cr,diff_cb;
	int k = 4, index, m = 0;
	for(i=0; i<vHeight; ++i){
		for (j=0; j<vWidth/2; ++j){
			index = (i * vWidth/2) + j;
			cr_buf_in = (0x00FF0000 & buffer_in[index]) >> 16;
			cb_buf_in = (0x000000FF & buffer_in[index]);
			y1_buf_in = buffer_in[index] >> 24;
			y0_buf_in = (0x0000FF00 & buffer_in[index]) >> 8;
			diff_y1 = (y1_buf_in -  bg_y_frame[m+1].mean) * (y1_buf_in -  bg_y_frame[m+1].mean);
			diff_y0 = (y0_buf_in - bg_y_frame[m].mean) * (y0_buf_in - bg_y_frame[m].mean);
//			diff_cr = (cr_buf_in -  bg_c_frame[m+1].mean) * (cr_buf_in -  bg_c_frame[m+1].mean);
//			diff_cb = (cb_buf_in - bg_c_frame[m].mean) * (cb_buf_in - bg_c_frame[m].mean);
//			if((diff_cr > (bg_c_frame[m+1].var*2)  && diff_cb > (bg_c_frame[m].var*2)) || (diff_y1 > (bg_y_frame[m+1].var*2)) && (diff_y0 > (bg_y_frame[m].var*2))){
			if((diff_y1 > (bg_y_frame[m+1].var*4)) && (diff_y0 > (bg_y_frame[m].var*4)) && (y0_buf_in > 220 && y1_buf_in > 220)){
...
void init_bg_frame(){
	...
	for(i=0; i<vHeight; ++i){
		for (j=0; j<vWidth/2; ++j){
			...
			cr = (0x00FF0000 & buffer_in[index]) >> 16;
			cb = (0x000000FF & buffer_in[index]);
			bg_c_frame[k].mean = cb;
			bg_c_frame[k].var = 10;
			bg_c_frame[k+1].mean = cr;
			bg_c_frame[k+1].var = 10;
			
			y1 = (0xFF000000 & buffer_in[index]) >> 24;
			y0 = (0x0000FF00 & buffer_in[index]) >> 8;
			bg_y_frame[k].mean = y0;
			bg_y_frame[k].var = 20;
			bg_y_frame[k+1].mean = y1;
			bg_y_frame[k+1].var = 20;
			k +=2;
...
void main( void )
{
	...
	while (1){
		...
		if ( dip0 == DIP_DOWN ){
			init_bg_frame();
			// Background detection Initialize
			while(total_frames < 30){
				total_frames++;
				bg_y_calculation();
		//		bg_color_calculation();
			}
			total_frames = 0;
		}
	//     StoreSum();
